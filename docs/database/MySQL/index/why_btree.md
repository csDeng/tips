...
![图片](./pics/why_btree/3dcb127877c7e77839404275279b136b.png)
...
![图片](./pics/why_btree/f01bb5e7e940231c4f39e7f1cfb449f3.png)
...
![请添加图片描述](./pics/why_btree/49cb4df3fb7d4accbc75f6970ff1d0ea.gif)
...
![图片](./pics/why_btree/f3089bdfe1a795a8142d4442bdb1e9a7.gif)

另外，二叉查找树解决了插入新节点的问题，因为二叉查找树是一个跳跃结构，不必连续排列。这样在插入的时候，新节点可以放在任何位置，不会像线性结构那样插入一个元素，所有元素都需要向后排列。

下面是二叉查找树插入某个节点的动图演示：

![请添加图片描述](./pics/why_btree/be13777664584209826f90d5d4128b7f.gif)
...
![请添加图片描述](./pics/why_btree/320c5c2653e74f9e9ebfcd8ca9681905.gif)
...
![图片](./pics/why_btree/8d5da86f2e41a0b641dbf45b23f7b7dd.gif)
...
![图片](./pics/why_btree/b2628d1248e41207a08871f7bfac3522.gif)

**不管平衡二叉查找树还是红黑树，都会随着插入的元素增多，而导致树的高度变高，这就意味着磁盘 I/O 操作次数多，会影响整体数据查询的效率**。

比如，下面这个平衡二叉查找树的高度为 5，那么在访问最底部的节点时，就需要磁盘 5 次 I/O 操作。

![图片](./pics/why_btree/2d26d30c953cd47c6ab637ad0eba2f99.png)

根本原因是因为它们都是二叉树，也就是每个节点只能保存 2 个子节点 ，如果我们把二叉树改成 M 叉树（M>2）呢？

比如，当 M=3 时，在同样的节点个数情况下，三叉树比二叉树的树高要矮。

![图片](./pics/why_btree/00fb73de7014a87958f1597345e9ef2f.png)
...
![图片](./pics/why_btree/9a96956de3be0614f7ec2344741b4dcc.gif)

我们来看看一棵 3 阶的 B 树的查询过程是怎样的？

![图片](./pics/why_btree/341cedca6863fcd0624febd835165ae2.gif)
...
![图片](./pics/why_btree/b6678c667053a356f46fc5691d2f5878.png)
...
![请添加图片描述](./pics/why_btree/25508b0cd9c44ef6937fdd737020a7f1.gif)
...
![请添加图片描述](./pics/why_btree/2be62679487640bbaac663fa96c7f35f.gif)

甚至，B+ 树在删除根节点的时候，由于存在冗余的节点，所以不会发生复杂的树的变形，比如下面这个动图是删除 B+ 树根节点的过程：

![请添加图片描述](./pics/why_btree/23730b5af987480fabff0f1d142a2b6c.gif)

B 树则不同，B 树没有冗余节点，删除节点的时候非常复杂，比如删除根节点中的数据，可能涉及复杂的树的变形，比如下面这个动图是删除 B 树根节点的过程：

![图片](./pics/why_btree/7552002f9b8195ab650d431bfe66cce2.gif)
...
![图片](./pics/why_btree/dd076212a7637b9032c97a615c39dcd7.png)
...
MySQL 是会将数据持久化在硬盘，而存储功能是由 MySQL 存储引擎实现的，所以讨论 MySQL 使用哪种数据结构作为索引，实际上是在讨论存储引使用哪种数据结构作为索引，InnoDB 是 MySQL 5.7 默认的存储引擎，它就是采用了 B+ 树作为索引的数据结构。
