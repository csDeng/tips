...
### 9. 什么是 ReadView，什么时候创建
ReadView是事务在进行快照读的时候生成的记录快照, 可以帮助我们解决可见性问题。
只有在select的时候才会创建ReadView。但在不同的隔离级别是有区别的:
> 在RC隔离级别下，是每个select都会创建最新的ReadView，而在RR隔离级别下，则是当事务中的第一个select请求才创建ReadView。
> 也正因为这样，RC 可以读取已提交的数据，RR 保证可重复读。
...
**MyISAM与InnoDB区别**
![image.png](./MySQL/1599812260233-ab712bb5-89a1-4b48-8232-cac49b5a0d28.png)
![image.png](./MySQL/1676429302864-68abe963-58e0-424c-9fb7-0ff6a27c5998.png)

### 2. MyISAM索引与InnoDB索引的区别？

- InnoDB 主键索引是聚簇索引，所以Innodb 要求所有的数据表都设置主键索引，如果没有设置，会默认生成。MyISAM所有索引都是是非聚簇索引。
...
### 3. 索引使用场景（重点）
#### where
![image.png](./MySQL/1599812307393-b9378fca-a484-4985-afd8-c846cfd0b8ac.png)
...
```
![image.png](./MySQL/1599812358362-5cd6f729-2d85-4d21-ab6b-e79980ebb802.png)
![image.png](https://cdn.nlark.com/yuque/0/2020/png/2517043/1599812372720-e9d957a1-fba5-4464-9c73-4e79d00abb80.png#averageHue=%23e6e7eb&height=79&id=UcpBj&originHeight=158&originWidth=1940&originalType=binary&ratio=1&rotation=0&showTitle=false&size=46818&status=done&style=none&title=&width=970)
...
#### join
![image.png](./MySQL/1599812416723-003f94ee-236a-4398-b169-0e5287089043.png)
#### 索引覆盖
...
![image.png](./MySQL/1677065671303-551c2fc9-814c-45bf-9030-e25f871421c0.png)
...
![image.png](./MySQL/1599812517065-0fcebe5a-0c71-4a3b-94f0-ebfb3febd4f9.png)
...
![image.png](./MySQL/1599812727867-7b926226-550f-4ff6-a013-37cbc0fa1aa9.png)
需要取消自增长再行删除：
...
- 基于非唯一性索引的检索
### 14. 百万级别或以上的数据如何删除
关于索引：由于索引需要额外的维护成本，因为索引文件是单独存在的文件,所以当我们对数据的增加,修改,删除,都会产生额外的对索引文件的操作,这些操作需要消耗额外的IO,会降低增/改/删的执行效率。所以，在我们删除数据库百万级别数据的时候，查询MySQL官方手册得知删除数据时，如果索引过多，会导致锁表（上行锁）需要的时间比较长。
...
![image.png](./MySQL/1679545459399-63f92edc-8210-4441-b227-90db7a8b1f81.png)
### 15. 前缀索引
...
![image.png](./MySQL/1599812882308-c4ebe2f1-aab6-4ca4-a71a-b15630b12730.png)
### 18. 使用B树的好处
B树可以在内部节点同时存储键和值，因此，把频繁访问的数据放在靠近根节点的地方将会大大提高热点数据的查询效率。这种特性使得B树在特定数据重复多次查询的场景中更加高效。
### 19. 使用B+树的好处
由于B+树的内部节点只存放键，不存放值，因此，一次读取，可以在内存页中获取更多的键，有利于更快地缩小查找范围。 B+树的叶节点由一条链相连，因此，当需要进行一次全数据遍历的时候，B+树只需要使用O(logN)时间找到最小的一个节点，然后通过链进行O(N)的顺序遍历即可。而B树则需要对树的每一层进行遍历，这会需要更多的内存置换次数，因此也就需要花费更多的时间
### 20. Hash索引和B+树索引有什么区别或者说优劣呢?
...
> 因为在 hash 索引中经过 hash 函数建立索引之后，索引的顺序与原顺序无法保持一致，不能支持范围查询。而B+ 树的的所有节点皆遵循(左节点小于父节点，右节点大于父节点，多叉树也类似)，天然支持范围。

- hash 索引不支持使用索引进行排序，原理同上。
- hash 索引不支持模糊查询以及多列索引的最左前缀匹配。原理也是因为hash函数的不可预测。AAAA和AAAAB的索引没有相关性。
...
- **非聚簇索引**：将数据存储于索引分开结构，索引结构的叶子节点指向了数据的对应行，myisam通 key_buffer  把索引先缓存到内存中，当需要访问数据时（通过索引访问数据），在内存中直接搜索索引，然后通过索引找到磁盘相应数据，这也就是为什么索引不在 key buffer 命中时，速度慢的原因

澄清一个概念：innodb 中，在聚簇索引之上创建的索引称之为辅助索引，辅助索引访问数据总是需要二次查找，非聚簇索引都是辅助索引，像复合索引、前缀索引、唯一索引，辅助索引叶子节点存储的不再是行的物理位置，而是主键值

### 23. 何时使用聚簇索引与非聚簇索引
![image.png](./MySQL/1599812933861-cfaa2b6a-6054-412d-bd40-6e08ae27693b.png)
### 24. 非聚簇索引一定会回表查询吗？
不一定，这涉及到查询语句所要求的字段是否全部命中了索引，如果全部命中了索引 ( 也就是达到了索引覆盖的效果 )，那么就不必再进行回表查询。
...
MySQL可以使用多个字段同时建立一个索引，叫做联合索引。在联合索引中，如果想要命中索引，需要按照建立索引时的字段顺序挨个使用，否则无法命中索引。（也就是最左前缀原则）
具体原因为:
...
![image.png](./MySQL/1599812960554-0af20f22-4c0f-4de9-82f2-615f5a12a68a.png)

1. **原子性（Atomic）：** 事务是最小的执行单位，不允许分割。事务的原子性确保动作要么全部完成，要么完全不起作用；
2. **一致性 (Consistent)：** 执行事务前后，数据保持一致，多个事务对同一个数据读取的结果是相同的；
3. **隔离性 (Isolation)：** 并发访问数据库时，一个用户的事务不被其他事务所干扰，各并发事务之间数据库是独立的；
4. **持久性 (Duration)：** 一个事务被提交之后。它对数据库中数据的改变是持久的，即使数据库发生故障也不应该对其有任何影响。
...
> ![](https://cdn.nlark.com/yuque/0/2022/png/21883277/1662104922882-7fff98a4-c5f1-44cd-ba1b-75e6c7e570fc.png#averageHue=%23c55a11&clientId=ud0097588-abd3-4&errorMessage=unknown%20error&from=paste&id=uaa0e6178&originHeight=271&originWidth=343&originalType=url&ratio=1&rotation=0&showTitle=false&status=error&style=none&taskId=u80143001-86ab-46bf-8b6d-9f148d34a70&title=)
...
> ![](https://cdn.nlark.com/yuque/0/2022/png/21883277/1662105052688-a8a616e4-d513-4746-9229-8700ca3753a4.png#averageHue=%23c55a11&clientId=ud0097588-abd3-4&errorMessage=unknown%20error&from=paste&id=u3ec1b181&originHeight=289&originWidth=355&originalType=url&ratio=1&rotation=0&showTitle=false&status=error&style=none&taskId=u7053e4f6-7b8c-4135-997e-f0695335f11&title=)
...
> ### 写入过程
> ![image.png](https://cdn.nlark.com/yuque/0/2022/png/21883277/1662106039560-1e136972-c682-4972-b26a-d82c800e4946.png#averageHue=%23f6f6f6&clientId=ub5528761-3116-4&errorMessage=unknown%20error&from=paste&height=420&id=ufbf365ff&originHeight=462&originWidth=1241&originalType=binary&ratio=1&rotation=0&showTitle=false&size=179664&status=error&style=none&taskId=u0b10f2f4-dfd6-45f2-b531-b1c8394c695&title=&width=1128.1817937291364)
> 注意点：
>
...
- **幻读(Phantom Read)**: 在一个事务的两次查询中**数据行**数不一致，例如有一个事务查询了几列(Row)数据，而另一个事务却在此时插入了新的几列数据，先前的事务在接下来的查询中，就会发现有几列数据是它先前所没有的。

    例子:小明修改了A,B数据,小红同时又插入了一条C数据,小明会感觉自己明明数据都改了,突然多出来一条,以为
    出现了幻觉自己漏了一条   。
![image.png](./MySQL/1681630402085-5c0d5878-1c30-486d-a792-90e9d74e2cb0.png)

### 4. 什么是事务的隔离级别？MySQL的默认隔离级别是什么？
为了达到事务的四大特性，数据库定义了4种不同的事务隔离级别，由低到高依次为Read uncommitted、Read committed、Repeatable read、Serializable，这四个级别可以逐个解决脏读、不可重复读、幻读这几类问题。
![image.png](./MySQL/1599813010754-f03f7267-050c-4739-8575-3adbb16b993f.png)
**SQL 标准定义了四个隔离级别：**
...
### 6. MVCC 是如何实现读已提交和可重复读的呢？
首先 MVCC 的读操作可以分解为以下4个步骤：

1. 获取事务自己事务ID,即trx_id。(这个也不是select的时候获取的，而是这个事务开启的时候获取的也就是begin的时候)
...
### 7. MVCC 可以解决幻读问题吗
首先 MVCC 在 RC 以及 RR 两个隔离级别应用，且RC明显是有幻读问题的。
经过实验，在RR中也存在幻读（不是数据行数的幻读而是数据插入时可能会遇到“鬼影”），但是我们可以通过当前读来解决。
![image.png](./MySQL/1681630527366-d57d13f1-1b78-40d3-af59-9424d905c7ac.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/21883277/1662104680604-f7cf0492-c102-46b3-940b-fe740b498712.png#averageHue=%23e8dddd&clientId=ud0097588-abd3-4&errorMessage=unknown%20error&from=paste&id=u6a1a116c&originHeight=693&originWidth=670&originalType=url&ratio=1&rotation=0&showTitle=false&size=54802&status=error&style=none&taskId=u30c0a1f8-f983-4c8f-a4f9-87e1ff6e98e&title=)
...
### SQL 约束有哪几种？
![image.png](./MySQL/1599813061558-c89e951f-7069-452c-9f18-84eea994be80.png)
...
R表
![image.png](./MySQL/1599813252597-4d4a4dd3-2208-4126-9a53-3228128b530f.png)
S表
![image.png](./MySQL/1599813279642-c5a4ec74-30f1-4222-a469-c8d6dc71ed24.png)

1. 交叉连接(笛卡尔积):
```sql
select r.*,s.* from r,s
```
![image.png](./MySQL/1599813324487-c8e26c5a-b85b-49b5-9755-56cac8801fae.png)
2.内连接结果：

```sql
select r.*,s.* from r inner join s on r.c=s.c
```
![image.png](./MySQL/1599813372348-d82da8b9-6e9d-4a22-9fd9-85e3c93400bb.png)
3.左连接结果：

```sql
select r.*,s.* from r left join s on r.c=s.c
```
![image.png](./MySQL/1599813419791-8603215a-aa86-4d7b-b65e-a537a062eb12.png)
4.右连接结果：

```sql
select r.*,s.* from r right join s on r.c=s.c
```
![image.png](./MySQL/1599813462311-894cf53d-0951-4cc6-b45e-5b03ed53f35c.png)
5.全表连接的结果（MySql不支持，Oracle支持)：

```sql
select r.*,s.* from r full join s on r.c=s.c
```
![image.png](./MySQL/1599813502878-47a6bf45-5efb-44e2-afa6-bdc7ef2f5797.png)
### 什么是子查询
...
三者都表示删除，但是三者有一些差别：
![image.png](./MySQL/1599813643329-87980e24-8ae6-4864-89d5-e14f3a1f7800.png)
...
![image.png](./MySQL/1599813669937-39f0f943-1c13-4a31-ac24-4ed6f8c1a693.png)
...
![image.png](./MySQL/1599813697636-542c3092-6292-481d-8e8e-20208d84183f.png)
...
![image.png](./MySQL/1599813817748-76cba05a-750d-462a-9815-802cf6f854a5.png)
...
- 数据库层面。这也是我们主要集中关注的(虽然收效没那么大),类似于`select * from table where age > 20 limit 1000000,10`这种查询其实也是有可以优化的余地的. 这条语句需要load1000000数据然后基本上全部丢弃,只取10条当然比较慢. 当时我们可以修改    `select * from table where id in (select id from table where age > 20 limit 1000000,10)`.这样虽然也load了一百万的数据,但是由于索引覆盖,要查询的所有字段都在索引中,所以速度会很快. 同时如果ID连续的好,我们还可以`select * from table where id > 1000000 limit 10`,效率也是不错的,优化的可能性有许多种,但是核心思想都一样,就是减少load的数据.
...
### 慢查询日志
![image.png](./MySQL/1599814010166-d7c067e1-d66f-4515-b194-ee4062964e57.png)
...
MySQL内部每秒能扫描内存中上百万行数据，相比之下，响应数据给客户端就要慢得多，使用尽可能少的查询是好的，但是有时将一个大的查询分解为多个小的查询是很有必要的。
...
- count(*)会 忽略所有的列，直接统计所有列数，不要使用count(列名)
...
![image.png](./MySQL/1599814270500-ac3fab7d-a992-4687-9c5d-8bfc787cf1e4.png)
**垂直拆分的优点：** 可以使得行数据变小，在查询时减少读取的Block数，减少I/O次数。此外，垂直分区可以简化表的结构，易于维护。
**垂直拆分的缺点：** 主键会出现冗余，需要管理冗余列，并会引起Join操作，可以通过在应用层进行Join来解决。此外，垂直分区会让事务变得更加复杂；

#### 垂直分表
把主键和一些列放在一个表，然后把主键和另外的列放在另一个表中
![image.png](./MySQL/1599814293905-eb8f7569-b383-41c1-b4ef-f89ca565653a.png)
...
![image.png](./MySQL/1599814320089-dbd3abf9-c6fe-4568-bda8-a3760da97f71.png)
...
![image.png](./MySQL/1599814354124-9478fa07-de18-4da9-aabd-70cba86a5394.png)
...
![image.png](./MySQL/1599814391747-d331eb22-aca5-4ad9-a953-aeb0c47b5e81.png)
### MySQL的复制原理以及流程
...
**复制过程**
![image.png](./MySQL/1599814419275-736212dd-86a8-4e87-a8c6-46a4bd24786a.png)
